...
export async function handleGet (req: NextApiRequest, res: NextApiResponse) {
    try {
         ...
         const {idStudent, startDate, endDate } = req.query;
        
         // Hacer la solicitud GET con el ID
         const response = await axios.get(`${process.env.URL_API_BACKEND}/v1/report/late_fee_borrow_student_interval?idStudent=${idStudent}&startDate=${startDate}&endDate=${endDate}`, {
            headers: {
