...
const BorrowRevenueInterval = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const data = await findBorrowRevenueInterval(formattedStarDate,formattedEndDate)
            const {totalRevenue, borrows} = data
        ...
        <LocalizationProvider dateAdapter={AdapterDateFns}>
            <Grid container spacing={6}>
                ...
                <Grid item xs={12} md={4}>
                    ...
                    <Typography variant='body2'>Listado de los prestamos del total de dinero recaudado en un intervalo de tiempo</Typography>
                </Grid>
                <Grid item xs={12}>
                    <Card>
                        Total de ganancias: {totalRevenue}
                    </Card>
                    <Card>
...
import { findBorrowReturnedToday, findBorrowRevenueInterval } from 'src/utils/apiUtils/borrow/requestBorrow'
import { useRouter } from 'next/router'
import { isTokenValid, redirectToLogin } from 'src/utils/helpers/jwtHelper'
BorrowRevenueInterval.getLayout = (page: ReactNode) => <EmployeeLayout>{page}</EmployeeLayout>

export default BorrowRevenueInterval